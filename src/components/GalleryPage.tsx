...
export const GalleryPage = (): ReactElement => {
  ...
  const end = start + 6;
  // Total
  const total = Blogs.filter((blog) => blog.category === Categories.GALLERY).reverse();
  // Total count
  const paginationCount = Math.ceil(total.length / 6);
  // Current view
  const galleryContents: Contents[] = total.slice(start, end);

  // Change handler
  const handleChange = (event: React.ChangeEvent<unknown>, value: number) => {
    navigate(`/contents/gallery/${value}`);
  };
